...
# Define TextBlob POS tagging function
def textblob_pos_tagging(text):
    blob = TextBlob(text)
    return [tag for _, tag in blob.tags]
...
for document in conll2003_dataset["train"]:
    ...
    # Concatenate tokens into a single string for TextBlob tagging
    text = " ".join(tokens)

    # Tag the text using TextBlob
    pred_tags = textblob_pos_tagging(text)
...
print("POS Tagging Metrics using TextBlob on CoNLL-2003 dataset:")
