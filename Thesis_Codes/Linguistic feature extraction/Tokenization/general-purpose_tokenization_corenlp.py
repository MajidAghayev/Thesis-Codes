...
# Download and initialize the CoreNLP model with Stanza for tokenization only
stanza.download('en')  # This only needs to be done once
nlp = stanza.Pipeline('en', processors='tokenize')  # Initialize with tokenization
...
# Define tokenization function using CoreNLP
def tokenize_text_with_corenlp(text):
    doc = nlp(text)
...
# Apply CoreNLP tokenization
data['tokenized_text'] = data['text'].apply(tokenize_text_with_corenlp)
...
print("\nTokenized Text Model Metrics using CoreNLP:")
