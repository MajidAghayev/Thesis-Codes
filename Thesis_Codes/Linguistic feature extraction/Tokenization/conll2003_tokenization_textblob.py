...
# Define TextBlob tokenization function
...
for idx, document in enumerate(conll2003_dataset['train']):
    ...
    # Concatenate tokens into a single string for TextBlob tokenization
    text = " ".join(gold_tokens)

    # Tokenize using TextBlob
    textblob_tokens = textblob_tokenize(text)

    # Calculate token-level accuracy, precision, recall, and F1 score
    token_level_accuracy, precision, recall, f1 = calculate_token_level_metrics(textblob_tokens, gold_tokens)
...
# Print results
print("Tokenization Metrics using TextBlob on CoNLL-2003 dataset :")
