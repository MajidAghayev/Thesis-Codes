...
# Initialize the Stanza pipeline with English language model for tokenization
...
# Define the text cleaning function using Stanza (CoreNLP)
def clean_text_with_stanza(text):
    # Tokenize and process the text with Stanza
    doc = stanza_pipeline(text.lower())

    # Extract words only (filtering out punctuation, etc.) and join into a cleaned string
    cleaned_text = ' '.join([word.text for sentence in doc.sentences for word in sentence.words if word.text.isalpha()])
    return cleaned_text
...
# Apply text cleaning
data['cleaned_text'] = data['sms'].apply(clean_text_with_stanza)
...
print("\nCleaned Text Model Metrics (using CoreNLP via Stanza):")
